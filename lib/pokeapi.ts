...
export async function fetchPokemonPage(
  ...
  const details = await Promise.all(
    data.results.map((item) =>
      request<DetailResponse>(item.url)
    )
  )
...
export async function fetchPokemonByName(
  ...
  const data = await request<DetailResponse>(
    `${POKEAPI_BASE}/pokemon/${name.toLowerCase().trim()}`
  )
...
export async function fetchPokemonByType(
  ...
  const data = await request<TypeResponse>(
    `${POKEAPI_BASE}/type/${type}`
  )

  const details = await Promise.all(
    data.pokemon.map((item) =>
      request<DetailResponse>(item.pokemon.url)
    )
  )
  ...
  return {
    ...
    image:
      data.sprites.other?.['official-artwork']
        ?.front_default || imageFor(data.id),
    height: data.height,
    weight: data.weight,
